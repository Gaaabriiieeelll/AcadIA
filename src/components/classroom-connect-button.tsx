...
export function ClassroomConnectButton({ reconnect = false }: { reconnect?: boolean }) {
  const [loading, setLoading] = useState(false);

  async function connectClassroom() {
    if (loading) return;
    setLoading(true);
    window.location.assign(new URL("/api/classroom/connect", window.location.origin).toString());
  }

  return (
    <button
      aria-busy={loading}
      className="classroom-connect-button"
      disabled={loading}
